...
class Model:
    def __init__(self, history_length=1, learning_rate=3e-4):
        ...
        z1 = tf.nn.conv2d(x_input, self.W_conv1, strides=[4, 4, 1, 1], padding='VALID')
        a1 = tf.nn.relu(z1)
        # second layer + relu: 
        self.W_conv2 = tf.get_variable("W_conv2", [4, 4, 32, 64], initializer=tf.contrib.layers.xavier_initializer())
        z2 = tf.nn.conv2d(a1, self.W_conv2, strides=[2, 2, 1, 1], padding='VALID')
        a2 = tf.nn.relu(z2)
        # third layer + relu:
        ...
        self.cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=output, labels=self.y_label))
