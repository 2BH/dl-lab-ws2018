...
class CNN():
    ...
    def _build_model(self, num_actions, lr, history_length, diff_history):
        ...
        width, height = 210//2, 160//2

        self.diff_history = diff_history

        if not diff_history:
            self.states_ = tf.placeholder(
                tf.float32, shape=[None, width, height, history_length + 1])
        else:
            self.states_ = tf.placeholder(tf.float32, shape=[None, width, height, 1])
            self.hstates_ = tf.placeholder(
                tf.float32, shape=[None, width, height, history_length])
